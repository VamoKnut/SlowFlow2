...
**Status:** Consolidated MVP specification for code generation  
**Date:** 2026-08-09  
**Language:** English
...
* Arduino Portenta H7
* `SlowFlowShieldRev1` custom shield as the only supported MVP shield profile
* PlatformIO in Visual Studio Code as the primary build, dependency, test, and development environment
* GitHub source control integrated directly with the VS Code workflow
* C++/Arduino-compatible firmware
* Persistent local configuration and telemetry buffer
* Wi-Fi and 4G connectivity
* MQTT application protocol
* Full Modbus sensor support in the MVP

The SlowFlow firmware shall reuse the same proven firmware libraries and implementation patterns as HASTIG wherever they are practical and compatible. New alternative libraries shall only be introduced where there is a concrete need. The HASTIG implementation shall be inspected before selecting firmware dependencies and versions.
...
The MVP shall implement a configurable empirical power-law model:
...
* `h` is the already calculated non-negative `EffectiveHeadMm` converted to metres;
* `Q` is calculated internally in cubic metres per second;
* `C` is an installation-specific empirical coefficient;
* `n` is an installation-specific empirical exponent.

The model configuration shall include at least:
...
No separate crest-offset term shall be included because the model input is already the effective head above the effective outlet threshold. User interfaces and APIs may convert discharge to litres per second. `C`, `n`, and the valid head range shall be calibrated from physical measurements for the actual installation.
...
ShieldType = SlowFlowShieldRev1
```

The MVP shall support exactly one shield profile: `SlowFlowShieldRev1`.
...
`SlowFlowShieldRev1` shall define the concrete pin mappings, buses, peripheral assignments, and hardware-driver selection. Additional shield profiles are outside MVP scope. Firmware shall reject any unsupported `ShieldType`.
...
The actuator has built-in end stops. The absolute drain-crest position sensor is the authoritative source of actual position.

Initial configurable commissioning defaults are:

```text
PositionDeadbandMm = 2
SlowApproachDistanceMm = 15
MinimumMovementPulseMs = 200
MaximumMovementPulseMs = 1000
MinimumPauseMs = 500
MaximumContinuousMovementSeconds = 120
NoMovementTimeoutSeconds = 10
MinimumDetectedMovementMm = 1
ControlEvaluationIntervalMs = 100
PositionSampleIntervalMs = 100
PositionFilterWindowSize = 5
```

These are initial commissioning values and shall remain configurable. They shall be verified and tuned on the physical prototype.
...
```text
ConfirmationTimeMs = 250
ClearStabilityMs = 10000
```
...
After the input becomes inactive, it shall remain inactive for `ClearStabilityMs` before the fault may clear. The actuator shall remain stopped while the control state is re-evaluated.
...
Initial configurable engineering plausibility defaults are:

```text
WaterLevelMm:          -20 to 350 mm
DrainCrestPositionMm:  -70 to 350 mm
ExternalTemperatureC:  -40 to 60 °C
```

These plausibility limits are separate from actuator movement limits and may be adjusted per drain node during commissioning.
...
Full Modbus sensor support is required in the MVP. The proven HASTIG Modbus implementation and libraries shall be reused where practical.

Modbus shall be treated as another source of a generic sensor `RawValue`. After acquisition, the same common sensor pipeline shall be used as for analog inputs:
...
Signed/unsigned interpretation shall be configurable through `RawValueType`. Byte and word order shall be configurable where required, particularly for 32-bit values.

Modbus raw values shall not use a separate engineering conversion path. Calibration/scaling, filtering, engineering plausibility limits, freshness, and quality reporting shall follow the same logic as analog sensors.
...
```text
SafeLoweringStepMm = 10
SafeLoweringStepIntervalSeconds = 60
```

This corresponds to a nominal target reduction of 10 mm per minute. The ordinary fixed crest-position controller shall move to each temporary target using all normal actuator protections.
...
```text
FrostTriggerTemperatureC = 2
FrostTriggerDelayMinutes = 10
FrostReleaseTemperatureC = 4
FrostReleaseDelayMinutes = 30
```

Frost protection shall activate when the selected valid temperature source remains at or below 2 °C for 10 minutes. It shall release only when the selected valid source remains at or above 4 °C for 30 minutes. Short variations between the trigger and release thresholds shall not change the frost state.

The selected temperature source must remain valid throughout the trigger or release delay period. If an enabled selected source is invalid, stale, missing, or implausible, a temperature-source fault is raised and safe lowering is activated.
...
The MVP local UI shall provide the following functions:
...
The display shall show at least:

* water level relative to drain datum;
* effective head relative to drain crest;
...
* Manual mode shall be entered explicitly from the local menu.
* Hold Up to move upward.
* Hold Down to move downward.
* Release the key to stop immediately.
* Left exits manual mode.
* Hardware interlocks always remain active.

A separate confirmed `Move to Bottom` action shall move the drain automatically to the mechanical minimum using the normal actuator protections.

### 15.3 Provisioning

Provisioning shall:

* start explicitly from the local menu, or automatically when network configuration is missing;
* create a temporary local Wi-Fi access point;
* show a QR code linking to a local configuration portal where the display supports it;
* allow configuration of Wi-Fi, APN, and MQTT connection settings;
* expire after approximately 15 minutes of inactivity;
* never embed permanent passwords, MQTT credentials, or other long-lived secrets in the QR code.

The backend remains the primary source for ordinary operational configuration.
...
```text
CommunicationRestartThresholdMinutes = 30
LongCommunicationLossSafeLoweringHours = 12
```

During short communication outages the drain node shall continue using its last valid normal mode and setpoints and shall buffer telemetry locally.

After 30 minutes without valid backend acknowledgement, the node shall attempt communication recovery and may perform the configured watchdog/deep restart if required. Persistent control state and setpoints shall survive restart. Recovery logic shall avoid an uncontrolled rapid restart loop.

After 12 hours without valid backend contact, `LongCommunicationLoss` shall activate and the node shall begin the normal gradual safe-lowering sequence. These thresholds shall remain configurable.
...
```text
MinimumBufferDurationDays = 10
MaximumLocalSampleCount = 14400
MaximumTelemetryBatchSize = 100
```

The local store shall be a persistent bounded ring buffer. Under the expected profile, 14,400 samples provides at least the required 10-day capacity and approximately 10 days even at an average of one stored sample per minute.

The buffer shall provide:

* unique persistent sample sequence;
* batched upload of at most 100 samples per normal telemetry message;
* acknowledgement-based deletion;
* idempotent backend ingestion;
* explicit overflow reporting.

Samples shall normally be removed only after backend acknowledgement. If the bounded buffer must overwrite unacknowledged samples, `TelemetryBufferOverflow` shall be raised and the lost sample-sequence interval/count shall be reported.
...
Backend offline detection shall use the configurable MVP default:

```text
DrainNodeOfflineThresholdMinutes = 25
```

With the normal approximately 10-minute complete-status interval, the backend shall activate `DrainNodeCommunicationLost` after 25 minutes without valid status or equivalent contact. The alarm shall clear automatically when valid communication resumes.
...
* estimated remaining storage capacity where calculable;
* calculated discharge where configured and valid;
...
Public pages shall use an explicit whitelisted snapshot DTO and may show:

* public roof/building display name;
* street address;
* exact geographic position/coordinates suitable for a future map overlay;
...
Internal database IDs shall never be exposed through public pages or public APIs.
...
* detailed private alarms;
* raw sensor values;
* user data.

Public exposure shall use explicit public DTOs and configurable display fields rather than serializing internal entities directly.
...
OTA capability remains an MVP requirement in principle, but the concrete Portenta H7 implementation shall be selected through a later technical prototype using available libraries and reference implementations.

The backend shall support the conceptual management of:
...
* deployment lifecycle state and audit history.

The selected OTA mechanism shall ultimately provide:

* download over the active network connection using an appropriate secure transport;
* compatibility validation before installation;
* integrity and authenticity verification using the selected mechanism;
* installation that does not leave the device permanently unusable after an interrupted or failed update;
* post-update health confirmation and an appropriate recovery strategy.
...
* concrete Portenta H7 OTA library;
* bootloader mechanism;
* flash/update partition layout;
* exact signature/authenticity implementation;
* rollback/recovery mechanism;
* key provisioning details.

Existing Portenta-compatible libraries and reference implementations shall be evaluated before these details are fixed.
...
```text
MaximumEntityRows = 10000
MaximumTimeSeriesRows = 50000
MaximumTimeSeriesRangeDays = 366
IntegrationApiReadRequestsPerMinute = 120
AutomationApiReadRequestsPerMinute = 120
AutomationApiWriteRequestsPerMinute = 30
PublicRequestsPerMinutePerIp = 60
```

Both row count and time range shall be enforced for time-series requests. Exceeding configured limits shall return a stable API error such as `400 Bad Request` for excessive query scope or `429 Too Many Requests` for rate limiting.

Request body size and request timeout shall remain configurable. A 1 MB maximum request body is an appropriate initial default for ordinary API requests; firmware upload endpoints may use separate limits.

### 32.4 Data retention

Backend telemetry retention shall default to three years and remain configurable.

The three-year automatic retention policy applies to:

* water-level samples;
* drain-crest-position samples;
* external-temperature samples.

Telemetry cleanup shall run in bounded batches to avoid long-running database locks.

The following shall not be subject to automatic expiry in the MVP:

* hardware-configuration versions;
* calibration history;
* desired-state and command history;
* alarms;
* audit records;
* drain-node installation history;
* firmware release and deployment history.

### 32.5 Logging

`ApplicationLogRetentionDays = 30` shall be the configurable MVP default for application-log retention.
...
Firmware that fails compatibility, integrity, or authenticity verification shall not be installed. OTA shall not start during blocked operational states. The concrete verification and recovery mechanism remains deferred pending the OTA technical prototype.

## 34\. Recommended implementation phases

1. Repository, solution scaffold, terminology, PlatformIO/VS Code firmware tooling, and GitHub baseline.
...
6. Firmware `SlowFlowShieldRev1`, analog and full Modbus sensors, storage, and actuator foundation.
...
14. OTA technical prototype, explicit implementation selection, and subsequent OTA implementation.
...
* inspect and reuse compatible HASTIG firmware libraries and implementation patterns before introducing alternatives;
...
* select or implement a concrete Portenta H7 OTA mechanism before the deferred OTA technical-prototype decision is explicitly resolved.

## 36\. Intentionally deferred items

Most previously open MVP requirements have now been resolved. The following details remain intentionally deferred and do not block the initial implementation phases:

1. **Production backup policy:** exact SQL Server backup frequency, backup retention schedule, off-host copy policy, and periodic restore-test interval. The deployment documentation shall still include the backup and restore procedures and clearly mark these values as deployment decisions.
